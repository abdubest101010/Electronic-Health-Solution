...
export async function POST(req: NextRequest) {
  ...
  if (!session || session.user.role !== "RECEPTIONIST") {
    console.log(
      "❌ [MarkAsPaid] Unauthorized access - Missing or invalid session:",
      session
    );
  ...
  try {
    console.log(
      `🔍 [MarkAsPaid] Checking lab orders for patient ${patientId} on ${date}...`
    );
    ...
    if (labOrders.length === 0) {
      console.warn(
        `❌ [MarkAsPaid] No ASSIGNED lab orders found for patient ${patientId} on ${date}`
      );
      ...
      console.log(
        `🔍 [MarkAsPaid] All lab orders for patient ${patientId} on ${date}:`,
        allLabOrders
      );
      return NextResponse.json(
        {
          error: `No ASSIGNED lab orders found for patient ${patientId} on ${date}`,
          allLabOrders,
        },
        { status: 404 }
      );
    ...
    console.log(
      `🔍 [MarkAsPaid] Updating ${labOrders.length} lab orders for patient ${patientId} on ${date} to PAID...`
    );
    ...
    console.log(
      `✅ [MarkAsPaid] Updated ${labOrders.length} lab orders for patient ${patientId} on ${date}`
    );
    return NextResponse.json({
      message: `Marked ${labOrders.length} lab orders as paid`,
    });
  } catch (error: any) {
