...
export async function GET(req: NextRequest) {
  ...
  if (!session || !["LABORATORIST", "DOCTOR"].includes(session.user.role)) {
    console.log(
      "❌ [LabResults GET] Unauthorized access - Missing or invalid session:",
      session
    );
    return NextResponse.json(
      { error: "Unauthorized: Laboratorist or Doctor only" },
      { status: 401 }
    );
  }
  ...
  if (!patientId || typeof patientId !== "string") {
    console.warn(
      "❌ [LabResults GET] Invalid or missing patientId:",
      patientId
    );
    return NextResponse.json(
